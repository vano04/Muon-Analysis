...
def add_common_experiment_args(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Optional override for the number of worker processes",
    )
...
def _to_hybrid_optimizer(name: str) -> str:
    return "muon_hybrid" if str(name).lower() == "muon" else str(name)


def _to_pure_optimizer(name: str) -> str:
    return "muon" if str(name).lower() == "muon_hybrid" else str(name)
...
def load_benchmark_from_args(
    args,
    *,
    muon_mode: str = "as_configured",
    default_run_name_suffix: str | None = None,
) -> BenchmarkConfig:
    benchmark = BenchmarkConfig.load_json(args.config)

    if args.run_name is not None:
        benchmark.run_name = args.run_name
    elif default_run_name_suffix:
        benchmark.run_name = f"{benchmark.run_name}{default_run_name_suffix}"

    if args.max_parallel_trials is not None:
        benchmark.max_parallel_trials = args.max_parallel_trials

    if muon_mode == "hybrid":
        benchmark.optimizers = _dedupe_preserve_order([_to_hybrid_optimizer(name) for name in benchmark.optimizers])
    elif muon_mode == "pure":
        benchmark.optimizers = _dedupe_preserve_order([_to_pure_optimizer(name) for name in benchmark.optimizers])
    elif muon_mode != "as_configured":
        raise ValueError(f"Unsupported muon_mode {muon_mode}")

    benchmark.validate()
    return benchmark


def benchmark_run_dir(benchmark: BenchmarkConfig) -> Path:
    return Path(benchmark.output_root) / f"run_{benchmark.run_name}"


def print_experiment_summary(run_dir: Path, summary: dict):
...
class _BenchmarkProgressDisplay:
    ...
    def set_phase(self, text: str):
        self.summary_line_2.set_description_str(f"phase: {text}")
...
def _run_tier(benchmark: BenchmarkConfig, tier: TierConfig, run_dir: Path, force: bool, verify_existing: bool, progress_bar: _BenchmarkProgressDisplay | None = None) -> dict:
    ...
    if progress_bar is not None:
        progress_bar.set_phase(f"{tier.name} preparing teacher/val/test")
    build_meta = build_teacher_and_eval(build_config, tier_dir, verify_existing=verify_existing and not force)

    def _on_train_seed_progress(seed_index: int, total: int, train_seed: int, status: str):
        if progress_bar is None:
            return
        progress_bar.set_phase(f"{tier.name} train batches {seed_index}/{total} seed={train_seed} {status}")

    build_train_tokens(
        build_config,
        tier_dir,
        train_seeds=tier.train_seeds,
        force=force,
        verify_existing=verify_existing and not force,
        progress_callback=_on_train_seed_progress,
    )
    if progress_bar is not None:
...
def run_experiment(benchmark: BenchmarkConfig, run_dir: Path, force: bool = False, verify_existing: bool = False):
    ...
    worker_slots = max(0, benchmark.max_parallel_trials)
